 ...
 * @returns Unsigned transaction ready for dual signing
 */
export async function buildUnshieldTx(
    ...
    feePayer: PublicKey
): Promise<{ transaction: VersionedTransaction; stealthSigner: Keypair }> {
    const lightConnection = getLightConnection();
    const standardConnection = getStandardConnection();
    ...
    try {
        ...
        console.log("[Nudge] Transaction built (unsigned, ready for dual signing)!");

        return { transaction, stealthSigner: signer };
